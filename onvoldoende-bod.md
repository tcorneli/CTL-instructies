```mermaid
graph TD
    A[Law 27A: Does offender's LHO want to accept the insufficient bid?] -->|Yes| B[Auction continues with no rectification.]
    A -->|No| C[Law 27B: It must be corrected by any sufficient bid or a pass]
    C --> D{Was IB corrected by the lowest sufficient bid specifying same denomination?}
    D -->|Yes| E[Law 27B1a: Auction continues with no further rectification. Laws 26 and 16C do not apply.]
    D -->|No| F{Was the IB replaced by a Comparable Call?}
    F -->|Yes| G[Law 27B1b: Auction continues with no further rectification.]
    F -->|No| H[Law 27B2: The offender's partner must pass WHENEVER it is his turn to call.]
    H --> I{Does offender become a defender?}
    I -->|No| J[No Lead restriction.]
    I -->|Yes| K[Law 26 Lead Restrictions will apply: Declarer may prohibit offender's partner from leading any suit not specified by the offender.]
```
